...
MIN_EPSILON = 0.10
STEPS = EPISODES * 200
DECAY = 0.000001 #LAMBDA?
GAMMA = 0.9
RANDOM_START_FRAMES = int(random.uniform(0, 45)) # wait N frames before start
...
class GameRunner:
    ...
    def run(self, render = None):
        ...
        #skip initial frames. random start
        for i in range(RANDOM_START_FRAMES):
                self._env.step(0)
                if render:
                    self._env.render()
        ...
        print("Step {}, Eps: {}".format(self._steps, self._eps))
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        for cnt in range(EPISODES):
            ...
            gr.run(render=render)
            score = gr._reward_store[-1]
            print('Episode {} of {}. Score = {}'.format(cnt, EPISODES, score))
